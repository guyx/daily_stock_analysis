...
class Config:
    ...
    # === Quant Hub 数据基座增强（A 股，本地 Parquet 数据源，可选） ===
    quant_hub_data_root: Optional[str] = None
    quant_hub_max_stale_days: int = 3
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            quant_hub_data_root=(os.getenv('QUANT_HUB_DATA_ROOT') or '').strip() or None,
            quant_hub_max_stale_days=parse_env_int(
                os.getenv('QUANT_HUB_MAX_STALE_DAYS'), 3,
                field_name='QUANT_HUB_MAX_STALE_DAYS', minimum=0,
            ),
